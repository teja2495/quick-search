...
class FilePreferences(context: Context) : BasePreferences(context) {
        ...
        fun getPinnedFileUris(): Set<String> =
                getPinnedStringItems(BasePreferences.KEY_PINNED_FILE_URIS)

        fun getExcludedFileUris(): Set<String> =
                getExcludedStringItems(BasePreferences.KEY_EXCLUDED_FILE_URIS)

        fun pinFile(uri: String): Set<String> =
                pinStringItem(BasePreferences.KEY_PINNED_FILE_URIS, uri)

        fun unpinFile(uri: String): Set<String> =
                unpinStringItem(BasePreferences.KEY_PINNED_FILE_URIS, uri)

        fun excludeFile(uri: String): Set<String> =
                excludeStringItem(BasePreferences.KEY_EXCLUDED_FILE_URIS, uri)

        fun removeExcludedFile(uri: String): Set<String> =
                removeExcludedStringItem(BasePreferences.KEY_EXCLUDED_FILE_URIS, uri)

        fun clearAllExcludedFiles(): Set<String> =
                clearAllExcludedStringItems(BasePreferences.KEY_EXCLUDED_FILE_URIS)

        fun getExcludedFileExtensions(): Set<String> =
                getStringSet(BasePreferences.KEY_EXCLUDED_FILE_EXTENSIONS)
        ...
        fun getShowHiddenFiles(): Boolean =
                prefs.getBoolean(BasePreferences.KEY_SHOW_HIDDEN_FILES, false)

        fun setShowHiddenFiles(show: Boolean) =
                prefs.edit().putBoolean(BasePreferences.KEY_SHOW_HIDDEN_FILES, show).apply()
