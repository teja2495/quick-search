...
class ContactPreferences(context: Context) : BasePreferences(context) {
    ...
    fun getPinnedContactIds(): Set<Long> =
            getPinnedLongItems(BasePreferences.KEY_PINNED_CONTACT_IDS)

    fun getExcludedContactIds(): Set<Long> =
            getExcludedLongItems(BasePreferences.KEY_EXCLUDED_CONTACT_IDS)

    fun pinContact(contactId: Long): Set<Long> =
            pinLongItem(BasePreferences.KEY_PINNED_CONTACT_IDS, contactId)

    fun unpinContact(contactId: Long): Set<Long> =
            unpinLongItem(BasePreferences.KEY_PINNED_CONTACT_IDS, contactId)

    fun excludeContact(contactId: Long): Set<Long> =
            excludeLongItem(BasePreferences.KEY_EXCLUDED_CONTACT_IDS, contactId)

    fun removeExcludedContact(contactId: Long): Set<Long> =
            removeExcludedLongItem(BasePreferences.KEY_EXCLUDED_CONTACT_IDS, contactId)

    fun clearAllExcludedContacts(): Set<Long> =
            clearAllExcludedLongItems(BasePreferences.KEY_EXCLUDED_CONTACT_IDS)

    fun getPreferredPhoneNumber(contactId: Long): String? {
        return prefs.getString("${BasePreferences.KEY_PREFERRED_PHONE_PREFIX}$contactId", null)
    }
    ...
    fun getLastShownPhoneNumber(contactId: Long): String? {
        return prefs.getString("${BasePreferences.KEY_LAST_SHOWN_PHONE_PREFIX}$contactId", null)
    }
    ...
    fun isDirectDialEnabled(): Boolean =
            getBooleanPref(BasePreferences.KEY_DIRECT_DIAL_ENABLED, false)

    fun setDirectDialEnabled(enabled: Boolean) {
        setBooleanPref(BasePreferences.KEY_DIRECT_DIAL_ENABLED, enabled)
    }

    fun hasSeenDirectDialChoice(): Boolean =
            getBooleanPref(BasePreferences.KEY_DIRECT_DIAL_CHOICE_SHOWN, false)
    ...
    fun isDirectDialManuallyDisabled(): Boolean =
            getBooleanPref(BasePreferences.KEY_DIRECT_DIAL_MANUALLY_DISABLED, false)
    ...
    fun getPrimaryContactCardAction(
            contactId: Long
        ...
        return serialized?.let {
            com.tk.quicksearch.search.contacts.models.ContactCardAction.fromSerializedString(it)
        }
    }
    ...
    fun getSecondaryContactCardAction(
            contactId: Long
    ): com.tk.quicksearch.search.contacts.models.ContactCardAction? {
        val serialized =
                prefs.getString(
