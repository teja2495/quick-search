...
fun ContentLayout(
    ...
    Column(modifier = modifier, verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        // Determine whether to show recent queries (when empty) or web suggestions (when query but
        // no results)
        ...
        if (hasQuery && !hasAnySearchResults && !showCalculator && !hasDirectSearchAnswer) {
            ...
            // and web suggestions API was called but returned no results or web suggestions are
            // disabled
            ...
            // Add delay before showing no results text to avoid flashing before web suggestions
            // load
        ...
        when {
            isReversed -> {
                // Keyboard-aligned (reversed): search results first (when hasQuery), then other
                ...
                if (hasQuery) {
                    ...
                    // Show inline search engines below search results when inline mode is enabled
                ...
                if (!isExpanded) {
                    ...
                    orderedSections.forEach { section ->
                        when (section) {
                            SearchSection.APPS -> {
                                // In reversed layout, show recent queries before apps (since apps
                                // will be at bottom)
                                ...
                                if (shouldRenderPinnedApps) {
                                    val appsContext = SectionRenderContext(shouldRenderApps = true)
                        ...
                        orderedSections.forEach { section ->
                            when (section) {
                                SearchSection.APPS -> {
                                    ...
                                    if (shouldRenderPinnedApps) {
                                        val appsContext =
                                                SectionRenderContext(shouldRenderApps = true)
                                        renderSection(SearchSection.APPS, pinnedParams, appsContext)
                                    }
