...
object DesignTokens {
    ...
    /**
     * Extra extra large shape - 28dp (Circle-like for search bar height) Use for: Search bar, Round
     * buttons
     */
    ...
    /**
     * Button corner radius - 24dp Note: M3 buttons often use full circle height (e.g. 40dp height
     * -> 20dp radius). 24dp radius suitable for 48dp height buttons.
     */
    ...
    /**
     * Search bar text and icon color Use for: Search input text, search icons on dark backgrounds
     */
    ...
    fun cardItemTopPadding(isFirstItem: Boolean) =
            if (isFirstItem) CardTopPadding else CardVerticalPadding

    /** Calculates bottom padding for card items */
    fun cardItemBottomPadding(isLastItem: Boolean) =
            if (isLastItem) CardBottomPadding else CardVerticalPadding
}
